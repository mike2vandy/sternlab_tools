...
def createHeader(sortedGroups, csq_fields, target_sams):
  first = "chrm,pos,filter,ref,alt,major,minor,minor_alt,end,svlen"
...
def main():
  ...
  des_to_get = ['Allele', 'Consequence', 'IMPACT', 'Gene', 'Feature_type', 'Feature', 'BIOTYPE', 'EXON', 'INTRON','STRAND']
  ...
  for record in vcf_file(region):
    ...
    if af < 0.5:
      ...
      major, minor = alt, ref
      minor_alt = False
    first = ','.join(map(str, [chrm, pos, filt, ref, alt, major, minor, minor_alt, end, svlen]))
