...
def createHeader(sortedGroups, samples):
    ...
    header = f"{first},{middle},{end},{','.join(samples)}"
    return(header)
...
samples = vcfR.samples
print(createHeader(sortedGroups, samples))
...
for line in vcfR:
    ...
    gts = ','.join(group_lists['all'])
    for consequence in csq:
        print(f"{first},{afs_str},{consequence},{gts}")
